...
Из фич: блог со списком/категориями/тегами и пагинацией, поиск по постам, локализация контента (ru как базовый язык, en — перевод), уровни доступа к статьям (`FREE` / `BASIC` / `PREMIUM`) и страница тарифов, SEO (sitemap, robots, JSON-LD, canonical/hreflang).

Подписки пока работают в демо-режиме: платёжный провайдер и оформление тарифа не подключены. Тестовые подписки и уровни доступа к статьям создаются сидом.
