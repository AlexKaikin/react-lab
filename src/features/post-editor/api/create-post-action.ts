...
import { createPost, isSlugTaken } from '@/entities/post'
...
export async function createPostAction(values: unknown) {
  ...
  if (await isSlugTaken(parsed.data.slug)) return { error: 'slugTaken' as const }

  await createPost(buildPostInput(parsed.data))
