...
import { createCategory, isCategorySlugTaken } from '@/entities/post-category'
...
export async function createCategoryAction(values: unknown) {
  ...
  if (await isCategorySlugTaken(parsed.data.slug)) return { error: 'slugTaken' as const }

  await createCategory(buildCategoryInput(parsed.data))
